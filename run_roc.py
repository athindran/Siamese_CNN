...
folder = "../athindran_96/input_data_resized"
...
#        quit()
        if step % 10 == 0:
            saver.save(sess, 'pedestrian_test.ckpt')
            #embed = siamese.o1.eval({siamese.x1: X_test[0:128][0]})
            #embed.tofile('embed.txt')
else:
    saver.restore(sess, 'pedestrian.ckpt')
    test_o1 = np.zeros((X_test.shape[0],2))
    test_o2 = np.zeros((X_test.shape[0],2))
    test_size = 8
    total_test_batches = int(X_test.shape[0]/test_size)

    for i in range(total_test_batches):
      s  = i * test_size
      e = (i+1) *test_size
      input1,input2,y =next_batch(s,e,X_test,y_test)
      test_o1[s:e],test_o2[s:e] = sess.run([siamese.o1, siamese.o2], feed_dict={siamese.x1: input1,siamese.x2:input2,siamese.y_: y})
     
    test_dist2 = np.sum((test_o1-test_o2)**2,1,keepdims=True)
    test_dist = np.sqrt(test_dist2)
   
    test_dist = test_dist.ravel()
    #correct = np.argwhere(np.logical_or(np.logical_and(np.less(test_dist,2.5),np.equal(y_test,1)),np.logical_and(np.greater(test_dist,2.5),np.equal(y_test,0))))
    #correct = correct.ravel()
    #print(correct.size)
    #print(X_test.shape[0])
    #print(correct.size/X_test.shape[0])
    
    thresholds = np.sort(test_dist.ravel())
    tpr = np.zeros((1,thresholds.size))
    fpr = np.zeros((1,thresholds.size))
    for i in range(thresholds.size):
      threshold = thresholds[i]
      tp = np.argwhere(np.logical_and(np.less(test_dist,threshold),np.equal(y_test,1)))
      fp = np.argwhere(np.logical_and(np.less(test_dist,threshold),np.equal(y_test,0)))
      tn = np.argwhere(np.logical_and(np.greater(test_dist,threshold),np.equal(y_test,0)))
      fn = np.argwhere(np.logical_and(np.greater(test_dist,threshold),np.equal(y_test,1)))
      tpr[0,i] = tp.size/(tp.size+fn.size)
      fpr[0,i] = fp.size/(fp.size+tn.size)
    plt.plot(fpr[0,:],tpr[0,:])
    plt.xlabel('False positive ratio')
    plt.ylabel('True positive ratio')
    plt.title('Receiver Operating Characteristic')
    plt.savefig('siamese_ROC.jpg')
    plt.close()
